...
class VersionUtils {
  ...
  static bool isNewerVersion({
    ...
    AppLogger.debug('Comparing versions: current=$currentVersion, new=$newVersion');
    ...
    if (currentVersion.contains('nightly') && newVersion.contains('nightly')) {
      ...
      AppLogger.debug('Both are nightly versions, use isNewerNightly instead');
      return false;
      ...
      AppLogger.debug('Nightly vs Stable comparison:');
      AppLogger.debug('  Current nightly: $currentVersion -> base: $currentBase');
      AppLogger.debug('  New stable: $newVersion -> base: $newBase');
      ...
      AppLogger.debug('  Base version comparison result: $baseVersionComparison (positive = stable newer)');

      final shouldUpdate = baseVersionComparison > 0;
      AppLogger.debug('  Should update from nightly to stable: $shouldUpdate');
      ...
      AppLogger.debug('Current is stable, new is nightly - not updating');
      return false;
    }
  ...
  static bool isNewerNightly({
    ...
    AppLogger.debug('Comparing nightly versions:');
    AppLogger.debug('  Current: $currentVersion');
    AppLogger.debug('  New: $newVersion');
    ...
    try {
      ...
      if (currentMatches.isNotEmpty) {
        ...
        AppLogger.debug('  Parsed current build time: $currentBuildTime');
      }
      ...
      if (newMatches.isNotEmpty) {
        ...
        AppLogger.debug('  Parsed new version build time: $newVersionBuildTime');
      }
    } catch (e) {
