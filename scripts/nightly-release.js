...
class NightlyReleaser {
    ...
    generateReleaseNotes(version, releaseInfo, nightlyBuild) {
        ...
        const apk = releaseInfo.files.find(f => f.type === 'APK');

        const commitLine = gitInfo.commitMessage
            ? gitInfo.commitMessage.split('\n')[0]
            : 'Unknown';

        const notes = `**${commitLine}**

Commit: \`${(gitInfo.commit || 'unknown').slice(0, 7)}\` · Branch: \`${gitInfo.branch || 'main'}\`

${apk ? `SHA256: \`${apk.checksum}\`` : ''}

Download the APK below. Enable "Install from unknown sources" in Android settings before installing.
`;
